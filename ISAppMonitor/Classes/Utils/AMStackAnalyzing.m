...
/*
 单个函数栈帧的地址划定范围
 1. ebp == fp(frame pointer) 指向栈帧的底部
 2. esp == sp(stack pointer) 指向栈帧的顶部
 */
typedef struct StackFrame_mo {
    const struct StackFrame_mo *const previous;
    const uintptr_t return_address; // 函数调用区域调用指令的下一条指令地址，供函数返回时继续执行
} StackFrame_mo;

// mo == moniter
typedef struct ThreadInfo_mo {
    double cpuUsage;
    integer_t userTime;
} ThreadInfo_mo;

@implementation AMStackAnalyzing

NSString* stackInfoInThread(thread_t pthread){
    ThreadInfo_mo threadInfo_mo = {0};
    
    thread_info_data_t threadInfo;
    mach_msg_type_number_t threadInfoCount = THREAD_INFO_MAX;
    
    kern_return_t kr = thread_info((thread_act_t)pthread, THREAD_BASIC_INFO, (thread_info_t)threadInfo, &threadInfoCount);
    if (kr == KERN_SUCCESS) {
        thread_basic_info_t threadBasicInfo = (thread_basic_info_t)threadInfo;
        if (!(threadBasicInfo->flags & TH_FLAGS_IDLE)) {
            threadInfo_mo.cpuUsage = threadBasicInfo->cpu_usage / 10;
            threadInfo_mo.userTime = threadBasicInfo->system_time.microseconds;
        }
    }
    
    uintptr_t buffer[100];
    int i = 0;
    NSMutableString *retStr = [NSMutableString stringWithFormat:@" Stack of thread: %u:\n CPU used: %.1f percent \n user time: %d second\n", pthread, threadInfo_mo.cpuUsage, threadInfo_mo.userTime];
     ...
     */
    _STRUCT_MCONTEXT machineContext; // 线程所有的栈指针
    //通过 thread_get_state 获取完整的 machineContext 信息，包含 thread 状态信息
    mach_msg_type_number_t state_count = mo_ThreadStateCountByCPU();
    kern_return_t kr_1 = thread_get_state(pthread, mo_ThreadStateByCPU(), (thread_state_t)&machineContext.__ss, &state_count);
    if (kr_1 != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", pthread];
    }
    
    // 通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = mo_MachInstructionPointerByCPU(&machineContext);
    ...
    // 链接地址
    uintptr_t linkRegisterPointer = mo_MachThreadGetLinkRegisterPointerByCPU(&machineContext);
    if (linkRegisterPointer) {
        buffer[i] = linkRegisterPointer;
        i++;
    }
    
    StackFrame_mo stackFrame = {0};
    // 通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = mo_MachStackBasePointerByCPU(&machineContext);
    if (framePointer == 0 || mo_MemCopySafely((void *)framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer.";
    }
    
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || mo_MemCopySafely(stackFrame.previous, &stackFrame, sizeof((stackFrame))) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    mo_Symbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; i++) {
        [retStr appendFormat:@"%@", mo_OutputLog(i, buffer[i], &symbolicated[i])];
    }
    [retStr appendFormat:@"\n"];
    return retStr;
}

#pragma mark - Build Stack Info
NSString* mo_OutputLog(const int entryNum, const uintptr_t address, const Dl_info* const dlInfo) {
...
kern_return_t mo_MemCopySafely(const void *const src, void *const dst, const size_t byteSize) {
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)byteSize, (vm_address_t)dst, &bytesCopied);
}

#pragma mark - Symbolicate 符号化函数
/*
 stackBuffer: buffer[i] 符号地址
 symbolsBuffer: 等待赋值的符号信息
 stackLength: i
 skippedEntries: 默认传入0
 */
void mo_Symbolicate(const uintptr_t* const stackBuffer, Dl_info* const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    if (!skippedEntries && i < stackLength) {
        mo_Dladdr(stackBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    for (; i < stackLength; i++) {
        mo_Dladdr(mo_InstructionAddressByCPU(stackBuffer[i]), &symbolsBuffer[i]);
    }
}
/*
 address: buffer[i] => stackFrame info
 info: &symbolsBuffer[i] start is empty
 
 // mach header 学习地址：https://zhuanlan.zhihu.com/p/24858664
 */
bool mo_Dladdr(const uintptr_t address, Dl_info* const info) {
    ...
    // 第一步：根据地址获取是哪个 image
    const uint32_t idx = mo_DyldImageIndexFromAddress(address);
    if (idx == UINT_MAX) {
        return false;
    }
    ...
    const uintptr_t segmentBase = mo_SegmentBaseOfImageIndex(idx) + imageVMAddressSlide;
    if (segmentBase == 0) {
        return false;
    }
    //
    ...
    uintptr_t cmdPointer = mo_CmdFirstPointerFromMachHeader(machHeader);
    if (cmdPointer == 0) {
        return false;
    }
...
uint32_t mo_DyldImageIndexFromAddress(const uintptr_t address) {
    //返回当前 image 数，这里 image 不是线程安全的，因为另一个线程可能正处在添加或者删除 image 期间
    const uint32_t imageCount = _dyld_image_count();
    const struct mach_header* machHeader = 0;
    //O(n2)的方式查找，考虑优化
    for (uint32_t iImg = 0; iImg < imageCount; iImg++) {
        //返回一个指向由 image_index 索引的 image 的 mach 头的指针，如果 image_index 超出了范围，那么返回 NULL
        machHeader = _dyld_get_image_header(iImg);
        if (machHeader != NULL) {
            //查找 segment command
            uintptr_t addressWSlide = address - (uintptr_t)_dyld_get_image_vmaddr_slide(iImg); // 通过地址相减获得偏移地址，为了跟segment内部的地址比较
            uintptr_t cmdPointer = mo_CmdFirstPointerFromMachHeader(machHeader);
            if (cmdPointer == 0) {
                continue;
            }
            for (uint32_t iCmd = 0; iCmd < machHeader->ncmds; iCmd++) {
                // 根据mach header的内存结构，首先就算是command区域
                const struct load_command* loadCmd = (struct load_command*)cmdPointer;
...
uintptr_t mo_CmdFirstPointerFromMachHeader(const struct mach_header* const machHeader) {
...
uintptr_t mo_SegmentBaseOfImageIndex(const uint32_t idx) {
    const struct mach_header* machHeader = _dyld_get_image_header(idx);
    //查找 segment command 返回 image 的地址
    uintptr_t cmdPtr = mo_CmdFirstPointerFromMachHeader(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
 ...
 */
uintptr_t mo_MachStackBasePointerByCPU(mcontext_t const machineContext) {
...
uintptr_t mo_MachInstructionPointerByCPU(mcontext_t const machineContext) {
...
uintptr_t mo_InstructionAddressByCPU(const uintptr_t address) {
#if defined(__arm64__)
...
mach_msg_type_number_t mo_ThreadStateCountByCPU() {
#if defined(__arm64__)
...
*/
thread_state_flavor_t mo_ThreadStateByCPU() {
#if defined(__arm64__)
...
uintptr_t mo_MachThreadGetLinkRegisterPointerByCPU(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
