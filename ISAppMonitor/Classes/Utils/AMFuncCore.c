...
static CallRecord *_CallRecords;
...
static inline uintptr_t pop_call_record() {
    ...
    if (cs->is_main_thread && _call_record_enabled) {
        ...
        if (cost > _min_time_cost && cs->index < _max_call_depth) {
            if (!_CallRecords) {
                _RecordAlloc = 1024;
                _CallRecords = malloc(sizeof(CallRecord) * _RecordAlloc);
            }
            _RecordNum++;
            if (_RecordNum >= _RecordAlloc) {
                _RecordAlloc += 1024;
                _CallRecords = realloc(_CallRecords, sizeof(CallRecord) * _RecordAlloc);
            }
            CallRecord *log = &_CallRecords[_RecordNum - 1];
            log->cls = pRecord->cls;
            log->depth = curIndex;
            log->sel = pRecord->cmd;
            log->time = cost;
...
void CallConfigMinTime(uint64_t us) {
    _min_time_cost = us;
}
void CallConfigMaxDepth(int depth) {
    _max_call_depth = depth;
}

CallRecord *GetCallRecords(int *num) {
    if (num) {
        *num = _RecordNum;
    }
    return _CallRecords;
